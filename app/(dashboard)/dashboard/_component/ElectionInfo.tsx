  ...
  return (
    <div className="grid gap-2 grid-cols-2 lg:grid-cols-5 w-full">
      <div className="flex flex-col">
        ...
        <span className="text-primary font-bold">{formatWithCommas(props.totalRegisteredVoters)}</span>
      </div>
      <div className="flex flex-col">
        <span>Total Accredicted Voters</span>
        <span className="text-primary font-bold">{formatWithCommas(props.totalAccredictedVoters)}</span>
      </div>
      <div className="flex flex-col">
        <span>Total Votes Count</span>
        <span className="text-primary font-bold">{formatWithCommas(props.totalVotesCount)}</span>
      </div>
      <div className="flex flex-col">
        <span>Total Valid Votes</span>
        <span className="text-primary font-bold">{formatWithCommas(props.totalValidVotes)}</span>
      </div>
      <div className="flex flex-col">
        <span>Total Rejected Votes</span>
        <span className="text-primary font-bold">{formatWithCommas(props.totalRejectedVotes)}</span>
      </div>
    </div>
  );
}
