...
export default function CollapsibleComponent({
  ...
  return (
    ...
    >
      <div className="flex border shadow-sm rounded-md items-center w-full bg-grey justify-between space-x-4 p-2 shadow-shadow">
        ...
        <CollapsibleTrigger asChild>
          <Button
            variant="outline"
            size="tab"
            className="w-[1.5rem] h-[1.5rem] bg-shadow text-accent"
          >
            <ChevronUp />
          </Button>
        </CollapsibleTrigger>
      ...
      <CollapsibleContent
        className={`space-y-2 overflow-hidden flex flex-col gap-1 
        ${isOpen ? 'animate-accordion-down pt-8' : 'animate-accordion-up'}`}
      >
        {children}
      </CollapsibleContent>
    </Collapsible>
