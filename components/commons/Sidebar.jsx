...
function Sidebar() {
    ...
    return (
        <div className={`${expanded ? 'w-[250px]' : 'w-[80px]'} bg-green-900 h-screen shadow p-2 py-6 flex flex-col space-y-4 ${expanded ? '' : 'items-center'} transition-all duration-200`}>
            <Link onClick={() => setLoading(true)} href="/">
                {
                    expanded ? (
                        <img src={'/assets/images/footerimage.png'} alt="" className="h-16" />
                    ) : (
            ...
            <MediaGallery onClick={() => displayPage(constants.MEDIA_GALLERY)} active={activeMenu == constants.MEDIA_GALLERY} />
    ...
    function Dashboard({onClick, active}){
        return (
            <div onClick={onClick} className={`${expanded ? 'w-full' :'w-[44px]'} h-[40px] p-2 rounded-[5px] border border-white/40 flex items-center ${active ? 'bg-white' : ''} hover:bg-white cursor-pointer group`}>
    ...
    function ElectionMap({onClick, active}){
        return (            
            <div onClick={onClick} className={`${expanded ? 'w-full' :'w-[44px]'} h-[40px] p-2 rounded-[5px] border border-white/40 flex items-center ${active ? 'bg-white' : ''} hover:bg-white cursor-pointer group`}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={active ? "#008000" : "white"} className="size-6 group-hover:text-[#008000] group-hover:fill-current">
    ...
    function MediaGallery({onClick, active}){
        return (            
            <div onClick={onClick} className={`${expanded ? 'w-full' :'w-[44px]'} h-[40px] p-2 rounded-[5px] border border-white/40 flex items-center ${active ? 'bg-white' : ''} hover:bg-white cursor-pointer group`}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={active ? "#008000" : "white"} className="size-6 group-hover:text-[#008000] group-hover:fill-current">
